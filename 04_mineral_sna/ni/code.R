...
data <- read_xlsx("C:\\Users\\ADMIN\\Desktop\\sna\\Nikel mineral database_20250803 (1).xlsx")
...
points(whkvy, col = 'red', pch=19)
text(whkvy,rownames(new_data)[importana_num], cex=0.8)
#dev.off()
...
#dev.off()
#points(mds_result[importana_num,])
rr <- mds_result[importana_num,]
rownames(new_data)[importana_num]
whkvy <- mds_result[importana_num,]
whkvy[1,] <- c(-0.3,0.1)
whkvy[2,] <- c(0.1,-0.25)
whkvy[3,] <- c(-0.26,-0.1)
whkvy[4,] <- c(0.365,0)
whkvy[5,] <- c(0.3834,-0.35)
whkvy[6,] <- c(0.25,-0.3)
whkvy[7,] <- c(0.21,0.1)
#par(bg = "gray80")
text(whkvy,rownames(new_data)[importana_num])
segments(
  x0 = rr[,1], y0 = rr[,2],   # 시작점 (원래 점 위치)
  x1 = whkvy[,1], y1 = whkvy[,2],  # 끝점 (라벨 위치)
  col = "black", lty = 1
)
...
table(tip_cols)
#png("dend2.png", width = 1600, height = 1200, res = 200)
plot(as.phylo(hc), type = "fan", tip.color = tip_cols, cex = 0.5)
plot(as.phylo(hc), type = "cladogram", tip.color = tip_cols, cex = 0.5)
plot(as.phylo(hc), type = "unrooted", tip.color = tip_cols, cex = 0.25)
#dev.off()
library(dendextend)
...
km$size
#png("mds_label_kmeans.png", width = 1600, height = 1200, res = 200)
...
#dev.off()
#plot(mds_result, col = km11$cluster, pch = 19, main = "K-means Clustering")
...
set.seed(1234)
mds_11 <- cmdscale(dist_mat1, k = 2)
set.seed(1234)
km11 <- kmeans(mds_11, centers = 3,nstart = 25)
km11$cluster
table(kmeans_label, km11$cluster)
randIndex(kmeans_label, km11$cluster)
table(kmeans_label, data$`(Biotic, Abiotic, Both)`)
